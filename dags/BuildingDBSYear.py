...
def clean_data(stocks_list):
    ...
    for i in range (len(stocks_list)):
        ...
        data = pd.read_csv(r''+ROOT_PATH+'/airflow/data/year/'+stocks_list[i]+'.csv')   
...
#CREATE DATAFRAME
def dataframe(stock):
    data = pd.read_csv (r''+ROOT_PATH+'/airflow/data/year/'+stock+'.csv')   
    df = pd.DataFrame(data)
    return df
...
def insertElements(connection, cursor, df, i):
    ...
    for row in df.itertuples():
        cursor.execute('''
                    INSERT INTO baseyear (id_stock,id_date, open ,close ,high ,low ,vloume)
